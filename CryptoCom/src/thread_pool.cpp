...
void thread_pool::create()
{
	...
	g_log->verbose("THREAD_POOL", "Allocating %d threads for thread pool.", thread_count);
...
void thread_pool::run()
{
	for (;;)
	{
		...
		{
			g_log->warning("THREAD_POOL", "Exception thrown while executing job:\n%s", e.what());
		}
	}

	g_log->verbose("THREAD_POOL", "Thread %d closing...", std::this_thread::get_id());
}